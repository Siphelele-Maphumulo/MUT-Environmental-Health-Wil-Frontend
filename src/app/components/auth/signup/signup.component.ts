...
export class SignupComponent implements OnInit {
  ...
  getFormValidationErrors(): string[] {
    ...
    if (controls['email'].hasError('required')) {
      ...
      errors.push('Email must end with @live.mut.ac.za'); // Updated error message
    }
  ...
  openPinDialog(): void {
    ...
    const askForCode = () => {
      const dialogRef = this.dialog.open(SignupCodeComponent, {
        width: '300px',
        disableClose: true,
      });
      ...
      dialogRef.afterClosed().subscribe((code) => {
        if (code || code.trim() !== '') {
          this.authService.validateSignupCode(code).subscribe(
            (isValid) => {
              if (isValid) {
                this.signupForm.addControl('code', this.fb.control(code)); // Add code to form

                this.authService
                  .studentSignup(this.signupForm.value)
                  .subscribe({
                    next: () => {
                      this.snackBar.open(
                        'Student signed  up successful!',
                        'Close',
                        {
                          duration: 3000,
                        }
                ...
                if (attempts >= maxAttempts) {
                  ...
                  const email = this.signupForm.get('email')?.value;
                  this.authService.blockSignupEmail(email).subscribe(() => {
                    this.snackBar.open(
                      'You have been blocked after 3 failed attempts. Please contact Secretary or Wil Coordinator.',
                      'Close',
                      { duration: 5000 }
                    );
