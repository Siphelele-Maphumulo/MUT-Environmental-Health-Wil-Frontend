...
import { StaffSignupCodeComponent } from './staff-sigup-code/staff-sigup-code.component';
...
export class StaffSignupComponent implements OnInit {
  ...
  getFormValidationErrors(): string[] {
    ...
    if (controls['signature'].hasError('required')) {
        ...
        this.authService.validateStaffCode(code).subscribe(
          (isValid) => {
            if (isValid) {
              ...
              this.authService.staffSignup(formData).subscribe({
                next: () => {
                  this.snackBar.open('Staff registration successful!', 'Close', {
                    duration: 3000,
