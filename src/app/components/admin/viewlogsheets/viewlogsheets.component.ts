...
export class ViewlogsheetsComponent implements OnInit {
  ...
  ngOnInit(): void {
    console.log('Initial logsheets data:', this.viewlogSheets);
  
    // Check if all logsheets have valid IDs
    this.viewlogSheets.forEach((log, index) => {
      console.log(`Logsheet ${index}: ID = ${log.id}, Type = ${typeof log.id}`);
    });
  ...
  private checkStudentNumberFilter(studentNumber: string | null | undefined): boolean {
    if (!studentNumber) return false; // Return false if studentNumber is null/undefined/empty
    const studentNumberFilter = this.studentNumberControl.value?.toString().toLowerCase() || '';
    if (!studentNumberFilter) return true;
    return studentNumber.toString().toLowerCase().includes(studentNumberFilter);
  }
...
getSignatureUrl(filename: string | null): string {
  ...
  async deleteLogSheet(id: number): Promise<void> {
    console.log('Delete method called with ID:', id, 'Type:', typeof id);
    // Validate ID
    if (!id || id <= 0) {
      console.error('Invalid logsheet ID:', id);
      this.showError('Invalid logsheet ID');
      return;
    }
    ...
    try {
      ...
      if (error.status === 404) {
        this.showError('Logsheet not found');
  ...
  async openSignDialog(log: LogSheet): Promise<void> {
    ...
    try {
      ...
      if (result && result.success) {
        console.log('Sign dialog closed with success:', result);
        await this.signLogSheet(log.id, result);
        
        // Show success message
        this.snackBar.open(
          `Logsheet signed successfully by ${result.supervisor_name}!`,
          'Close',
        ...
        console.log('Sign dialog closed with error:', result);
        this.snackBar.open(
          result.message || 'Failed to sign logsheet',
          'Close',
          { duration: 5000, panelClass: ['error-snackbar'] }
        );
      }
    } catch (error) {
      console.error('Error in sign dialog:', error);
